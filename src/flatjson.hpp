...
namespace flatjson {
    ...
    enum e_fj_token_type: std::uint8_t {
        FJ_TYPE_INVALID = 0, FJ_TYPE_STRING, FJ_TYPE_NUMBER, FJ_TYPE_BOOL, FJ_TYPE_NULL, FJ_TYPE_OBJECT,
        FJ_TYPE_OBJECT_END, FJ_TYPE_ARRAY, FJ_TYPE_ARRAY_END
    };

    inline const char *fj_token_type_name(e_fj_token_type t) {
        static const char *strs[] = {
            "INVALID", "STRING", "NUMBER", "BOOL", "NULL", "OBJECT", "OBJECT_END", "ARRAY", "ARRAY_END"
        };
    ...
    enum e_fj_error_code {
        FJ_EC_OK = 0, FJ_EC_INVALID = -1, FJ_EC_INCOMPLETE = -2, FJ_EC_NO_FREE_TOKENS = -3, FJ_EC_KLEN_OVERFLOW = -4,
        FJ_EC_VLEN_OVERFLOW = -5, FJ_EC_CHILDS_OVERFLOW = -6
    };
        ...
        std::size_t fj_utf8_char_len(CharT ch) {
            switch (static_cast<std::uint8_t>(ch)) {
                ...
                default:
                    ...
                    return 1; // safe fallback, avoids UB even with NDEBUG
            }
        }
...
#undef _FJ_CASE_40
        ...
        conv_to(const char *ptr, std::size_t len) {
            ...
            std::uint64_t v = 0;
            switch (len) {
                case 20: v = v + (str[len - 20] - '0') * 10000000000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 19: v = v + (str[len - 19] - '0') * 1000000000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 18: v = v + (str[len - 18] - '0') * 100000000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 17: v = v + (str[len - 17] - '0') * 10000000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 16: v = v + (str[len - 16] - '0') * 1000000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 15: v = v + (str[len - 15] - '0') * 100000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 14: v = v + (str[len - 14] - '0') * 10000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 13: v = v + (str[len - 13] - '0') * 1000000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 12: v = v + (str[len - 12] - '0') * 100000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 11: v = v + (str[len - 11] - '0') * 10000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 10: v = v + (str[len - 10] - '0') * 1000000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 9: v = v + (str[len - 9] - '0') * 100000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 8: v = v + (str[len - 8] - '0') * 10000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 7: v = v + (str[len - 7] - '0') * 1000000ull;
                    __FLATJSON__FALLTHROUGH;
                case 6: v = v + (str[len - 6] - '0') * 100000ull;
                    __FLATJSON__FALLTHROUGH;
                case 5: v = v + (str[len - 5] - '0') * 10000ull;
                    __FLATJSON__FALLTHROUGH;
                case 4: v = v + (str[len - 4] - '0') * 1000ull;
                    __FLATJSON__FALLTHROUGH;
                case 3: v = v + (str[len - 3] - '0') * 100ull;
                    __FLATJSON__FALLTHROUGH;
                case 2: v = v + (str[len - 2] - '0') * 10ull;
                    __FLATJSON__FALLTHROUGH;
                case 1: v = v + (str[len - 1] - '0') * 1ull;
                    __FLATJSON__FALLTHROUGH;
                default: break;
            }
...
#define __FLATJSON__CUR_CHAR(p) \
    ((fj_skip_ws(p)), (p->js_cur >= p->js_end ? ((char)-1) : *(p->js_cur)))
        ...
        template<parser_mode M, typename Iterator>
        int fj_parse_value(fj_parser<Iterator> *p, Iterator *ptr, std::size_t *size, e_fj_token_type *toktype,
                           fj_token<Iterator> *parent);
        ...
        int fj_parse_array(fj_parser<Iterator> *p, fj_token<Iterator> *parent) {
            ...
            __FLATJSON__CONSTEXPR_IF(M == parser_mode::parse) {
                ...
                if (startarr->__parent) {
                    __FLATJSON__CHECK_OVERFLOW(startarr->__parent->__childs, __FLATJSON__CHILDS_TYPE,
                                               FJ_EC_CHILDS_OVERFLOW);
        ...
        int fj_parse_object(fj_parser<Iterator> *p, fj_token<Iterator> *parent) {
            ...
            __FLATJSON__CONSTEXPR_IF(M == parser_mode::parse) {
                ...
                if (startobj->__parent) {
                    __FLATJSON__CHECK_OVERFLOW(startobj->__parent->__childs, __FLATJSON__CHILDS_TYPE,
                                               FJ_EC_CHILDS_OVERFLOW);
    ...
    template<typename Iterator>
    inline void fj_init(fj_parser<Iterator> *parser, Iterator beg, Iterator end, fj_token<Iterator> *tokbuf,
                        std::size_t toksize) {
        // root token
    ...
    template<typename Iterator>
    inline fj_parser<Iterator> fj_make_parser(fj_token<Iterator> *tokbuf, std::size_t toksize, Iterator beg,
                                              Iterator end) {
    ...
    inline std::size_t fj_get_tokens(const fj_token<Iterator> *toks, std::size_t num, std::size_t indent,
                                     void *userdata, fj_gather_cb_t cb) {
    ...
    template<typename Iterator>
    inline std::size_t fj_tokens_to_stream(int fd, const fj_token<Iterator> *toks, std::size_t num,
                                           std::size_t indent = 0) {
    ...
    template<typename Iterator>
    inline std::size_t fj_tokens_to_stream(std::FILE *stream, const fj_token<Iterator> *toks, std::size_t num,
                                           std::size_t indent = 0) {
    ...
    template<typename Iterator>
    inline std::size_t fj_tokens_to_stream(std::ostream &stream, const fj_token<Iterator> *toks, std::size_t num,
                                           std::size_t indent = 0) {
    ...
    template<typename Iterator>
    inline std::size_t fj_tokens_to_buf(const fj_token<Iterator> *toks, std::size_t num, char *buf, std::size_t size,
                                        std::size_t indent = 0) {
    ...
    private:
        ...
        struct tokens_iterator_impl {
            ...
            tokens_iterator_impl &operator++() {
                m_cur = iterate_until_next_direct_child(m_beg, m_beg->end(), m_cur);
                return *this;
            }
    ...
    public:
        ...
        fjson at(const const_iterator &it) const { return fjson{m_storage, std::addressof(*it), it->end()}; }
    ...
    private:
        ...
        find(std::size_t idx) const {
            ...
            if (idx >= static_cast<__FLATJSON__CHILDS_TYPE>(m_beg->__childs - 1)) {
                // one for END token
                return {nullptr, nullptr};
            }
