...
extern "C" {
int force_update(const size_t now, const size_t all) {
    Fl::wait(1.0 / 60.);
...
//     return 0;
// }
int progress_func(void *clientp,
                  const curl_off_t TotalToDownload,
                  const curl_off_t NowDownloaded,
                  curl_off_t TotalToUpload,
                  curl_off_t NowUploaded) {
    if (TotalToDownload > 0) {
        const auto progress =
                static_cast<float>(
                    static_cast<double>(NowDownloaded) /
    ...
    return 0; // 0 = continue, non-zero = abort transfer
}
...
const std::vector<BuildInfo> &Manager::getInfo() {
    if (buffer.empty()) {
        ...
        std::transform(buffer.cbegin(), buffer.cend(), versions.begin(),
                       [](auto &buildInfo) { return buildInfo.version; });
...
const BuildInfo &Manager::getCandidate() {
    ...
    const auto it = std::find_if(getInfo().begin(), getInfo().end(),
                                 [&](auto &member) -> bool {
                                     return
...
void Manager::unpack() {
    ...
    const std::string file = (std::filesystem::temp_directory_path() / getCandidate().name).string();

    const char *args[4]{"seven_z", "x", file.c_str()};

    std::filesystem::current_path(installDir);

    const std::filesystem::path mingw(std::filesystem::current_path() /
    ...
    const auto res = seven_z(force_update, 3, args);
...
void Manager::download() {
    ...
    auto &candidate = manager.getCandidate();
    ...
    for (auto &header: manager.headers_strings)
    ...
    curl_easy_setopt(http_handle, CURLOPT_XFERINFODATA, this); // or your userdata
    ...
    if (dataFile) {
        ...
        curl_easy_setopt(http_handle, CURLOPT_WRITEDATA, static_cast<void *>(dataFile));
    } else {
...
void Manager::downloading() {
    if (CURLMcode mc = curl_multi_perform(multi_handle, &(still_running)); !mc) {
...
void Manager::fillBuffer(std::vector<BuildInfo> &buff, const std::string &readBuffer) {
    const flatjson::fjson data{readBuffer.c_str(), readBuffer.c_str() + readBuffer.length()};
...
// Fl_RGB_Image *Manager::logo(const bool box) {
...
Fl_RGB_Image *Manager::logo() {
    static auto img =
            new Fl_PNG_Image("", getLogo().pointer, static_cast<int>(getLogo().size));
    return img;
}

std::string Manager::createIcon() const {
...
// std::string Manager::createBat() const {
//     std::string file_name = folderNames[downloadCandidate.architecture == Arcs::x86_64].data();
//     file_name += "/mingw.bat";
//     FILE *file = fopen(file_name.c_str(), "wb");
//     const std::string bat = "@echo off\n"
//                             "set PATH=" + manager.installDir +
...
bool Manager::getSjlj() {
    const auto it = std::find_if(buffer.begin(), buffer.end(), [&](auto &member) {
...
bool Manager::hasMcfForSelection() {
    // Make sure buffer+versions are loaded
    const auto &vers = getVersions();
    if (vers.empty())
        return false;

    // version index is safe because SelectInfo defaults to 0,
    // but still guard if something changes later.
    if (downloadCandidate.version >= vers.size())
        return false;

    const std::string &v = vers[downloadCandidate.version];

    return std::any_of(buffer.begin(), buffer.end(), [&](const BuildInfo &m) {
        return m.version == v
               && m.architecture == downloadCandidate.architecture
               && m.multithreading == MgRs::mcf;
    });
}
...
bool Manager::isMcfSelected() const {
    return downloadCandidate.multithreading == MgRs::mcf;
}
